...
function ProcessRequest () {
    ...
    try {
        ...
        # Disable SSL validation temporarily for the current request
        $webRequestSession = New-Object Microsoft.PowerShell.Commands.WebRequestSession
        $webRequestSession.CertificateValidationCallback = { $true }

        if ($Method.ToLower() -eq 'post' -or $Method.ToLower() -eq 'put' -or $Method.ToLower() -eq 'delete') {
            Write-Debug $url
            Write-Debug $method
            Write-Debug $body
            $response = Invoke-WebRequest -Uri $url -Method $Method -Body $Body -Headers $Header -ContentType $ContentType -WebSession $webRequestSession -ErrorAction Stop
            Write-Debug $response
        }
        elseif ($Method.ToLower() -eq 'get') {
            $response = Invoke-WebRequest -Uri $url -Headers $Header -ContentType $ContentType -WebSession $webRequestSession -ErrorAction Stop
        }
